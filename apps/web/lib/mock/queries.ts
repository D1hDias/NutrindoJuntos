/**
 * MOCK DATA QUERIES
 * Queries que usam dados mock estáticos para desenvolvimento
 */
 ...
 */
type MockResponse<T> = PaginatedResponse<T>
 ...
 * Buscar equipe
 */
export async function getEquipe(): Promise<MockResponse<any>> {
  const { MOCK_EQUIPE } = await import('../mock-data')
...
export type { MockResponse, PaginatedResponse }